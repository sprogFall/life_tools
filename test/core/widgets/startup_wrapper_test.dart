...
class MockSyncConfigService extends ChangeNotifier
...
class MockWifiService extends WifiService {
  final NetworkStatus status;

  MockWifiService(this.status);
...
void main() {
  testWidgets('StartupWrapper 当 autoSyncOnStartup=true 时应触发同步并显示成功 Toast', (
    tester,
  ) async {
  ...
  testWidgets('StartupWrapper 启动自动同步：私网模式但未连接 WiFi 时不应提示失败 Toast', (
    tester,
  ) async {
    final mockSyncConfigService = MockSyncConfigService();
    final config = SyncConfig(
  ...
  testWidgets('StartupWrapper 当 autoSyncOnStartup=false 时不应触发同步', (
    tester,
  ) async {
    final mockSyncConfigService = MockSyncConfigService();
    final config = SyncConfig(
