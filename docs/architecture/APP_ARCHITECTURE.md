...
同步快照还会额外加入一个非页面型 provider：

| toolId | 名称 | 来源 | 用途 |
| --- | --- | --- | --- |
| `app_config` | 应用配置 | `AppConfigSyncProvider` | 同步 AI 配置、同步配置、对象存储配置、AI 调用历史和全局设置 |
...
当前调用观测由 `AiCallHistoryService` 承担，记录最近调用、用量与耗时；小蜜会把这些信息展示给用户，配置和历史也通过 `app_config` 参与同步/备份。
...
- 查看工具级快照数据
- 对指定用户或指定工具进行编辑
- 编辑 `app_config` 中的 AI、同步、对象存储和全局设置
- 查看同步记录与历史快照，配合后端回退能力排障
- 分析工作记录工时树、归属画布和柱状图
