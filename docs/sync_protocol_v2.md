...
- 客户端每次同步都会导出所有支持同步的工具数据（`ToolSyncProvider.exportData()`），并 POST 到 `/sync/v2`
...
v2 协议就是为此规则提供**明确、可扩展**的出入参与决策流程。
...
- **统一协议入口**：客户端与服务端仅保留 `/sync/v2`，避免回退分支与双接口维护成本
...
- `force_decision`：可选，仅接受 `use_server` / `use_client`，用于用户确认覆盖方向后的强制重试
...
- 只调用 `/sync/v2`；服务端若未实现该接口则直接失败
- v2 下客户端会计算 `client_is_empty`（基于 `tools_data[*].data` 的深度判空）
- 客户端会在工具快照之外注入 `app_config`，同步 AI 配置、同步配置、对象存储配置、AI 调用历史和全局设置
