...
## 0) 选择图片（推荐方式：ImageSelector）

项目提供了 `ImageSelector` 工具类（`lib/core/utils/image_selector.dart`），封装了以下最佳实践：
- **自动压缩**：移动端默认压缩至 1080p / 80% 质量，大幅减少流量与内存占用
- **跨平台适配**：移动端用 `image_picker`，桌面端/Web 用 `file_picker`
- **自动清理**：自动处理 Android `file_picker` 产生的临时缓存文件

```dart
import 'package:life_tools/core/utils/image_selector.dart';

// 单选
final PickedImageBytes? picked = await ImageSelector.pickSingle();

// 多选
final List<PickedImageBytes> list = await ImageSelector.pickMulti();

if (picked != null) {
  print(picked.filename);
  print(picked.bytes.length);
}
```
...
// 3. 只保存 key（后续展示/缓存/导入导出都用这个 key）
...
3. **Android + file_picker：必须处理临时复制文件**
   - 若使用 `ImageSelector`，**无需关心此条**（内部已自动处理）。
   - 若手动使用 `file_picker`，必须在读取后调用 `FilePicker.platform.clearTemporaryFiles()`，避免相册出现重复图片。

4. **大文件注意**
   - 若使用 `ImageSelector`，**无需关心此条**（移动端已自动压缩）。
   - `uploadBytes(...)` 需要一次性把文件读入内存；若手动上传，请务必先压缩或限制大小.
